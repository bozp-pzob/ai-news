...
class ConfigUpdater {
  ...
  updateConfigs(guildChannels, configs) {
    ...
    for (const [configFile, configData] of configs) {
      ...
      for (const source of discordSources) {
        ...
        if (!guildId) {
          console.log(`⚠️  Cannot determine guild ID for source ${source.name} in ${configFile}`);
          continue;
        }

        const guildData = guildChannels.get(guildId);
        if (!guildData) {
          console.log(`⚠️  No checklist data found for guild ${guildId} (${source.name})`);
          continue;
        }
    ...
    console.log(`📊 Summary: ${totalUpdates} configuration files will be updated\n`);
   ...
   */
  saveConfigs(configs) {
    console.log('💾 Saving updated configurations...');
  ...
  generateSummary() {
    ...
    console.log(`   📄 Configuration files updated: ${this.changes.size}`);
...
async function main() {
  ...
  try {
    console.log(`🚀 Configuration Update Tool ${updater.dryRun ? '(DRY RUN MODE)' : ''}\n`);
    ...
    if (configs.size === 0) {
      console.log('❌ No Discord configurations found to update');
      process.exit(1);
    }
    ...
    if (!hasChanges) {
      console.log('✅ Configuration update complete (no changes needed)');
      return;
    }
